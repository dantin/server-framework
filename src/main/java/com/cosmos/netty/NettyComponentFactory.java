...
public abstract class NettyComponentFactory {
    ...
    public static void scanHeartBeatHandlerClass(Setting setting) {
        ...
        try {
            List<Class<?>> classes = ClassUtils.getAllSubClass(setting.getBasePackage(), IdleStateAwareChannelHandler.class);
    ...
    public static void scanProtocolHandlerClass(Setting setting) {
        // 扫描继承IdleStateAwareChannelHandler的心跳类
        try {
            List<Class<?>> classes = ClassUtils.getAllSubClass(setting.getBasePackage(), ProtocolHandler.class);
