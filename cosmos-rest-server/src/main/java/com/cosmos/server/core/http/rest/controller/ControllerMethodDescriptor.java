...
import com.cosmos.server.commons.annotations.RequestBody;
import com.cosmos.server.commons.annotations.RequestParam;
...
public class ControllerMethodDescriptor {
    ...
    public ControllerMethodDescriptor(String uri, ControllerClassDescriptor clazz, Method method) {
        ...
        for (int i = 0; i != total; i++) {
            if (paramsTypes[i] == HttpSession.class) {
                ...
                params[i].annotationType = MethodParams.AnnotationType.HTTP_SESSION;
            } else {
                params[i] = new MethodParams(annotations[i][0], paramsTypes[i]);
                if (params[i].annotation instanceof Header) {
                    params[i].annotationType = MethodParams.AnnotationType.HEADER;
                } else if (params[i].annotation instanceof RequestParam) {
                    params[i].annotationType = MethodParams.AnnotationType.REQUEST_PARAM;
                } else if (params[i].annotation instanceof RequestBody) {
                    params[i].annotationType = MethodParams.AnnotationType.REQUEST_BODY;
                } else if (params[i].annotation instanceof PathVariable) {
                    params[i].annotationType = MethodParams.AnnotationType.PATH_VARIABLE;
                    String name = ((PathVariable) params[i].annotation).value();
                    // find out the index of correspond variable
                    ...
                    if (params[i].urlPathIndex == -1)
                        throw new IllegalArgumentException(
                                String.format("%s[%s] is not found around %s()",
                                        PathVariable.class.getSimpleName(), name, method.getName()));
                } else {
    ...
    /**
     * Invoke registered business method.
     *
     * @param context http context
     * @return result object
     * @throws ControllerParamsNotMatchException
     * @throws ControllerParamsParsedException
     */
    ...
    /**
     * Resolve method parameter with input {@link HttpContext}.
     *
     * @param context http context
     * @return resolved parameter result
     * @throws ControllerParamsNotMatchException
     * @throws ControllerParamsParsedException
     */
    ...
    /**
     * Parse parameter with input parameter.
     *
     * @param clazz class
     * @param value value
     * @param serialize whether the value has been serialized
     * @param context http context
    ...
    /**
     * Get the wrapped business method.
     *
    ...
    /**
     * Method parameters which include parameter class type and parameter annotation.
     *
     * @author BSD
     */
